...
export const getInsights = asyncHandler(async (req: Request, res: Response) => {
  ...
  const queryBuilder = insightRepo
    ...
    .leftJoinAndSelect("insight.industry", "industry")
      .leftJoinAndSelect("insight.author", "author");

  // Apply filters
  if (industryId) {
    queryBuilder.andWhere("insight.industryId = :industryId", { industryId });
  }
...
export const createInsight = authAsyncHandler(async (req: MulterRequest, res: Response) => {
  ...
  if (subscribers.length > 0) {
    ...
    const industryName = savedInsight.industry?.name;
    await Promise.all(
      subscribers.map((s) =>
...
export const getInsightsByIndustry = asyncHandler(async (req: Request, res: Response) => {
  ...
  const insights = await insightRepo.find({
    where: { industry: { id: industryId }, isActive: true },
    relations: ["industry", "author"],
